...
\begin{itemize}
\item{clc} -- 清空命令窗口
\item{clear} - 清空工作空间, 清除指定变量用 \mcode{clear var1 var2}
\item{close} - 关闭窗口, 比如绘图窗口太多, 关闭所有绘图窗口 \mcode{close all}
...
\item{deal} -- 多个变量同时赋值, \\ \mcode{C = {rand(3) ones(3,1) eye(3) zeros(3,1)}; [a,b,c,d] = deal(C{:});}
\end{itemize}